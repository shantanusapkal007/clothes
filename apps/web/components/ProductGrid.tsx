  ...
  if (products.length === 0) {
    return (
      <motion.div 
        ...
        className="p-8 text-center text-secondary border border-dashed border-white/50 rounded-[28px] bg-white/40 backdrop-blur-md"
      >
    ...
    >
      {products.map((product) => (
        <motion.div
          ...
          key={product.id}
          className="group flex cursor-pointer flex-col rounded-3xl p-2 transition-colors hover:bg-white/40"
          onClick={() => onAdd(product)}
        >
          <div className="relative mb-3 aspect-[3/4] overflow-hidden rounded-[20px] bg-surface-container-low ring-1 ring-white/50 shadow-[0_8px_20px_rgb(0,0,0,0.03)] transition-transform duration-500 group-hover:-translate-y-1 group-hover:shadow-[0_15px_35px_rgb(0,0,0,0.06)]">
            <img
              alt={product.name}
            ...
            <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/5 transition-colors flex items-center justify-center">
              <div className="bg-white/90 backdrop-blur-xl w-12 h-12 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-[0_8px_20px_rgba(119,68,32,0.15)] transform translate-y-4 group-hover:translate-y-0 duration-400 ease-[cubic-bezier(0.23,1,0.32,1)] block">
          ...
          <div className="px-1.5">
            ...
            <div className="flex justify-between items-center mt-1.5">
              ...
              <p className="inline-flex items-center rounded-full bg-white/60 px-2 py-0.5 text-[10px] font-bold tracking-widest text-on-secondary-container">
                {product.stock}
              </p>
            </div>
          </div>
